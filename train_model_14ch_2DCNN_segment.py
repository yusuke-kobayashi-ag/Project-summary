...
# ディレクトリの作成
os.makedirs('./results/14ch_2DCNN_segment/distributions', exist_ok=True)
os.makedirs('./results/14ch_2DCNN_segment/models', exist_ok=True)
os.makedirs('./results/14ch_2DCNN_segment/evaluation', exist_ok=True)
os.makedirs('./results/14ch_2DCNN_segment/residual', exist_ok=True)

def create_model(input_shape):  # input_shape = (11, 5, 14, 2)
...
if __name__ == "__main__":
    ...
    plt.grid(True)
    plt.axis([-1, 1, -1, 1])
    plt.savefig('./results/14ch_2DCNN_segment/distributions/data_distribution.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'./results/14ch_2DCNN_segment/residual/residual_plots.png')
    plt.close()

    # モデルの保存
    valence_model.save(f'./results/14ch_2DCNN_segment/models/valence_model.keras')
    arousal_model.save(f'./results/14ch_2DCNN_segment/models/arousal_model.keras')
    ...
    # 結果の保存
    results_df.to_csv(f'./results/14ch_2DCNN_segment/evaluation/results.csv', index=False)
