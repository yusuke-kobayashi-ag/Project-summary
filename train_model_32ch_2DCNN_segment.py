...
# ディレクトリの作成
os.makedirs('./results/32ch_2DCNN_segment/distributions', exist_ok=True)
os.makedirs('./results/32ch_2DCNN_segment/models', exist_ok=True)
os.makedirs('./results/32ch_2DCNN_segment/evaluation', exist_ok=True)
os.makedirs('./results/32ch_2DCNN_segment/residual', exist_ok=True)

def create_model(input_shape):
    inputs = layers.Input(shape=input_shape)
    x = layers.Conv2D(filters=100, kernel_size=(1,2), activation="tanh", padding='same')(inputs)
    x = layers.BatchNormalization()(x)
    
    #x = layers.MaxPooling2D(pool_size=(1,2))(x)
    #x = layers.Dropout(0.1)(x)
    #x = layers.Conv2D(filters=40, kernel_size=(1,2), activation="tanh", padding='same')(x)
    #x = layers.BatchNormalization()(x)
    #x = layers.Dropout(0.1)(x)
...
if __name__ == "__main__":
    ...
    plt.grid(True)
    plt.axis([-1, 1, -1, 1])
    plt.savefig('./results/32ch_2DCNN_segment/distributions/data_distribution.png')
    plt.close()
    ...
    plt.tight_layout()
    plt.savefig(f'./results/32ch_2DCNN_segment/residual/residual_plots.png')
    plt.close()

    # モデルの保存
    valence_model.save(f'./results/32ch_2DCNN_segment/models/valence_model.keras')
    arousal_model.save(f'./results/32ch_2DCNN_segment/models/arousal_model.keras')
    ...
    # 結果の保存
    results_df.to_csv(f'./results/32ch_2DCNN_segment/evaluation/results.csv', index=False)
